...
import { DIRECTION, TIME_RANGE, ROAD_SECTION, TIME1_TYPE } from '../config';
import { eventTypeList } from '../TableItem/api';

const EventTypeC = props => {
    const {
        getChild,
        getDefaultData = () => {},
        getRightType,
    ...
    useEffect(() => {
        getDataF();
    }, [datetime, timeLevel, curDirection, pageParams, roadName]);
    return (
        <div className={Styles.TableModule}>
            <div className={Styles.title}>
                <span className={Styles.TitleLeft}></span>
                <span className={Styles.TitleText}>事件高发类型排名</span>
            </div>
...
export default React.memo(EventTypeC);
