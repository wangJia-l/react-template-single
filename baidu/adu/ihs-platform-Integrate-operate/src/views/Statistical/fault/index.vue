<template>
    <div class="home">
        <div class="page-item">
            <div class="content-title">故障告警列表</div>
                        ...
                        <el-row>
                            ...
                            <el-col :span="6">
                                <el-form-item label="系统选择">
                                    <el-select
                                        v-model="form.systemType" placeholder="监控"
                                        @change="selectChange"
                                    >
                                        <el-option
                            ...
                            <el-col :span="6">
                                <el-form-item label="故障状态" prop="status">
                                    <el-select
                                        v-model="form.status" placeholder="全部"
                                        @change="selectChange"
                                    >
                                        <el-option label="已处置（运维平台确认工单为已处置）" value="1"/>
                                        <el-option label="未处置" value="2"/>
                                    </el-select>
                                </el-form-item>
                            </el-col>
                        </el-row>
                        <el-row>
                            <el-col :span="6">
                                <el-form-item label="故障原因">
                                    <el-select
                                        v-model="form.alarmReason" placeholder="全部"
                                        @change="selectChange"
                                    >
                                        <el-option
...
<script>
import {faultList} from './servies';
import {seletListType} from '../repair/json';
import {DEVICE_LOCATION, DEVICE_DEVICE} from '../../DeviceView/json';
import {reporting, alarmSourceList, defaultTableTitleList} from './json';
...
export default {
    data() {
        ...
        return {
            ...
            tableListTitle: defaultTableTitleList,
            seletList: [
                {
                    title: '全部',
                    value: '0',
    ...
    methods: {
        open() {
            this.$confirm(`请确认导出数据${this.pagination.total}条`, {
                ...
                fileDownLoad({
                    url: '/ihs/monitor/device/statistics/exportFaultAlarmList',
                    param: {
                        ...
                        alarmReason: this.form.alarmReason,
        ...
        async handClickView(row) {
            const {deviceId} = row;
            // window.localStorage.setItem('brandList', JSON.stringify(data.brand));
        ...
        async getlist() {
            ...
            const data = await faultList(params);
            this.stationForm = data.records.map(item => {
                return {
                    ...item,
                    deviceLocation: this.dicStory.DEVICE_LOCATION[item.deviceLocation],
                    alarmSource: this.dicStory.reporting[item.alarmSource],
                    alarmReason: this.dicStory.alarmSourceList[item.alarmReason],
                    deviceType: this.dicStory.DEVICE_DEVICE[item.deviceType],
                    status: item.status === 1 ? '已处置' : '未处置',
                };
            });
            this.pagination = {
                total: data.total,
